 ...
 * @transaction
 */
function CreateDiagHistory(createDiagHistoryTx) {
 ...
 * @transaction
 */
function CreateProcedureHistory(createProcedureHistoryTx) {
 ...
 * @transaction
 */
function CreateDrugHistory(createDrugHistoryTx) {
 ...
 * @transaction
 */
function CreateCheckupHistory(createCheckupHistoryTx) {
 ...
 * @transaction
 */
function CreateVaccination(createVaccinationTx) {
 ...
 * A transaction processor for new permission request
 * 
 * @param {com.depa.blockchain.core.CreateXray} createXrayTx - A new permission request to be process
 * @transaction
 */
function CreateXray(createXrayTx) {
    return genericPermissionAddForCreate(XRAY_ASSET, createXrayTx)
}
 ...
 * @transaction
 */
function UpdateDiagHistory(updateDiagHistoryTx) {
 ...
 * @transaction
 */
function UpdateProcedureHistory(updateProcedureHistoryTx) {
 ...
 * @transaction
 */
function UpdateDrugHistory(updateDrugHistoryTx) {
 ...
 * @transaction
 */
function UpdateCheckupHistory(updateCheckupHistoryTx) {
 ...
 * A transaction processor for new permission request
 * 
 * @param {com.depa.blockchain.core.UpdateXray} updateXrayTx - A new permission request to be process
 * @transaction
 */
function UpdateXray(updateXrayTx) {
    return genericPermissionMaintainForUpdate(XRAY_ASSET, updateXrayTx)
}
...
function genericPermissionAddForCreate(assetName, createTx) {
    ...
    return getAssetRegistry(assetName)
        .then(function (assetRegistry) {
            ...
            for (var i = 0; i < newAsset.patient.authorizedHcpPermissionRequest.length; i++) {
        ...
        .then(function (protectedAssets) {
            ...
            protectedAssets.forEach(function(asset) {
                if (permissionTransaction.permissionType == 'GRANT') {
                    ...
                    asset.authorized.push(hcp);
                } else if (permissionTransaction.permissionType == 'REVOKE') {
                    positionOfHcpInArray = asset.authorized.indexOf(hcp);
                    asset.authorized.splice(positionOfHcpInArray, 1);
                }
                storedProtectedAssetAssetRegistry.update(asset);
            });
        });
}
